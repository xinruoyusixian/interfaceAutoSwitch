...
m = Map("network_switcher", "网络切换器配置", 
    "<h3>使用说明</h3>" ..
    "<ol>" ..
    "<li><b>全局设置:</b> 在下方配置 Ping 目标 IP、成功次数及其他网络检查参数。</li>" ..
    "<li><b>接口配置:</b> 添加您需要监控和切换的网络接口 (例如 <code>wan</code>, <code>wwan</code>)，并设定它们的优先级 (Metric) 和主备关系。</li>" ..
    "<li><b>定时任务:</b> (可选) 配置在特定时间自动执行的切换任务。</li>" ..
    "<li><b>保存配置:</b> 完成设置后，点击页面底部的“保存并应用”。</li>" ..
    "<li><b>启动服务:</b> 前往“概览”页面，点击“刷新状态”并确保服务已启动。</li>" ..
    "</ol>" ..
    "<p>这是一个智能的网络接口切换器，支持自动故障切换和定时切换功能。</p>")
